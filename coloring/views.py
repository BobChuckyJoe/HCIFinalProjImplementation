...
from coloring.models import Author, Drawing
...
def get_drawing_by_authorname(name):
  drawing = None

  if Drawing.objects.filter(author_name = name).exists():
    drawing = Drawing.objects.get(author_name=name)
  else:
    drawing = Drawing(author_name=name, title="Drawing Title", points="[]")
    drawing.save()
...
def index(request, authorname="DefaultAuthor"):
  ...
  if request.POST: 
    ...
    drawing = get_drawing_by_authorname(authorname)
